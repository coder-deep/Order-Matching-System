...
import com.oms.service.IClientTotalBuySellService;
import com.oms.service.IClientTotalSellService;
...
public class Controller {
	...
	@Autowired
	IClientTotalBuySellService ictbss;
	@Autowired
	IClientTotalSellService ictss;
	
	@PostMapping(value = "auth")
	public boolean getauth(@RequestBody Map<String,String> json)
	{
		List<Auth> l=ias.findAll();
		String un=json.get("username");
		int f=0,i;
		for(i=0;i<l.size();i++) {
			if(l.get(i).getCustodian_id().equals(un)) {
				f=1;
				break;
			}
		}
		...
		if(l.get(i).getPassword().equals(json.get("password"))) {
			return true;
		}
		return false;
	}
	...
	public Instrument getinstrumentbyid(@PathVariable("instrument_id") String instrument_id)
	{
...
//	}
	@GetMapping(value = "client_holdings/{client_id}")
	public List<Instrument> getinstrumentsHoldByClient(@PathVariable("client_id") String client_id)
	{
		return iis.getinstrumentsbyclient(client_id);
	}
	...
	public List<Client> getclientsbycustodian(@PathVariable("custodian_id") String custodian_id)
	{
		System.out.println(ics.getclientsbycustodian(custodian_id));
	...
	@GetMapping(value = "getClientNameByID/{id}",produces = "text/plain")
	public ResponseEntity<String> getClientNameByID(@PathVariable("id") String id)
	{
		Client c = ics.findById(id);
		return new ResponseEntity<String>(c.getClient_name(), HttpStatus.OK);
	}
	@GetMapping(value = "totalbuy/{custodian_id}")
	public List<ClientTotalBuy> getclientbuysellbycustodian(@PathVariable("custodian_id") String custodian_id)
	{
		System.out.print("at totalbuy sell");
		return ictbss.getclientbuysellbycustodian(custodian_id);
	}
	@GetMapping(value = "totalsell/{custodian_id}")
	public List<ClientTotalSell> getClientSell(@PathVariable("custodian_id") String custodian_id)
	{
		System.out.print("at totalbuy sell");
		return ictss.getTotalSell(custodian_id);
	}
